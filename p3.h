...
#include <sys/types.h>
#include <unistd.h>
#include <sys/processor.h> /* These two for processor_bind */
...
#define ARRIVAL_DELAY() sleep((random_r()%MAXLATENESS)+1)
#define READING() sleep((random_r()%2)+1)
#define WRITING() sleep((random_r()%2)+1)
#define PROCESSING() sleep((random_r()%2)+1)
